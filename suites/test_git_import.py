...
def git_import_for_invalid_git_repo_url(driver):
    git_import_page.import_git_repo(
        ...
        constants.git_import_branch)
    check_that(
        "Invalid git repo url error message", utilities.get_text_by_css(
            driver, locators.REPO_URL_INVALID_ERROR_CSS), contains_string(
            constants.repo_url_invalid_error_message))
...
def git_import(driver):
    ...
    utilities.verify_and_accept_confirmation_modal(
        driver,
        locators.GIT_IMPORT_REQUEST_SUBMITTED_TITLE,
    ...
    check_that(
        "All modules are imported", search_page.count_of_modules_with_the_source_name(
            driver, git_import_repo_Name), equal_to(
            constants.git_import_repo_modules_count))
