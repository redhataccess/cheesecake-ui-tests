...
class test_view_module(Screenshot):
    ...
    def authenticated_user_view_unpublished_module(self):
        ...
        # If the title is not found on the first page, search for the title and then click
            ...
            search_page.search_for_module_and_click(
                self.driver, constants.unpublished_module)
        check_that("URL", self.driver.current_url, contains_string(
        ...
        check_that("Button", utilities.get_text_by_css(
            self.driver, locators.MODULE_DISPLAY_PREVIEW_BUTTON_CSS), contains_string("Preview"))
        check_that("Publish status", utilities.get_text_by_xpath(
            self.driver, locators.MODULE_DISPLAY_PUBLISH_STATUS_XPATH), contains_string("Not published"))
        check_that("Module display page title", utilities.get_text_by_css(
            self.driver, locators.MODULE_DISPLAY_TITLE_CSS), contains_string(constants.unpublished_module))
    ...
    def view_on_portal_link_test(self):
        utilities.click_element_by_css_selector(
            self.driver, locators.VIEW_ON_PORTAL_LINK_CSS)
        ...
        try:
            check_that("Module content displayed on the Customer Portal", utilities.find_element_by_css(
                self.driver, locators.MODULE_BODY_ON_PORTAL_CSS).is_displayed(), is_(True))
        except TimeoutException as e:
            raise e
        finally:
