...
class test_publish_module(Screenshot):
    ...
    def product_info_on_view_page(self):
        utilities.click_element_by_css_selector(self.driver, locators.MODULE_DISPLAY_PREVIEW_BUTTON_CSS)
        time.sleep(3)
        utilities.switch_to_latest_tab(self.driver)
        # get updated and published date from view page and convert into desired format- (DD Month YYYY)
        test_publish_module.updated_date_view_page = (utilities.get_text_by_css(
            self.driver, locators.UPDATED_DATE_ON_PORTAL_CSS)).strip("Updated ")
        test_publish_module.published_date_view_page = (utilities.get_text_by_css(
            self.driver, locators.PUBLISHED_DATE_ON_PORTAL_CSS)).strip("Published ")

        # adding checks to verify product name,version,updated and published date on view page
        check_that("Product name reflected on view page", utilities.get_text_by_css(
            self.driver, locators.PRODUCT_NAME_CSS), contains_string(constants.product_name.upper()))
        check_that("Product version reflected on view page", utilities.get_text_by_css(
            self.driver, locators.PRODUCT_VERSION_CSS), contains_string(constants.product_version))
    ...
    def product_info_on_customer_portal(self):
        ...
        time.sleep(5)
        utilities.switch_to_latest_tab(self.driver)
        if utilities.get_text_by_css(self.driver, locators.MODULE_NOT_FOUND_CSS) == constants.module_not_found:
            ...
            check_that("Product name reflected on customer portal", utilities.get_text_by_css(
                self.driver, locators.PRODUCT_NAME_CSS), contains_string(constants.product_name.upper()))
            check_that("Product version reflected on customer portal", utilities.get_text_by_css(
                self.driver, locators.PRODUCT_VERSION_CSS), contains_string(constants.product_version))
