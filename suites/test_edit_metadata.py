...
@lcc.test(
    "Warning should be displayed on Edit Metadata modal when no data is entered")
def edit_metadata_blank_data(driver):
    ...
    # If the title is not found on the first page, search for the title and
    # then click
    except TimeoutException as e:
        search_page.search_for_module_and_click(
            driver, constants.module_to_be_published)
    utilities.click_element_by_css_selector(
        driver, locators.EDIT_METADATA_DROPDOWN_CSS)
    utilities.click_element_by_css_selector(
        driver, locators.EDIT_METADATA_BUTTON_CSS)
    check_that(
        "Edit metadata modal title", utilities.get_text_by_css(
            driver, locators.EDIT_METADATA_MODAL_TITLE_CSS), contains_string(
            constants.edit_metadata_modal_title))
    utilities.click_element_by_css_selector(
        driver, locators.EDIT_METADATA_SAVE_CSS)
    check_that(
        "Warning displayed", utilities.get_text_by_css(
            driver, locators.WARNING_ALERT_CSS), contains_string(
            constants.edit_metadata_modal_warning))
    utilities.click_element_by_css_selector(
        driver, locators.CLOSE_WARNING_ALERT_CSS)
...
@lcc.test(
    "Warning should be displayed on Edit Metadata modal when Usecase field is blank")
...
def edit_metadata_successfully(driver):
    ...
    utilities.click_element_by_css_selector(
        driver, locators.EDIT_METADATA_SAVE_CSS)
    check_that(
        "Success message is displayed", utilities.get_text_by_css(
            driver, locators.UPDATE_SUCCESS_MESSAGE_CSS), contains_string(
            constants.success_message))
    check_that(
        "Product name reflected on displayed module page",
        utilities.get_text_by_css(
            driver,
